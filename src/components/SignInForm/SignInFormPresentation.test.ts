...
import SignInFormPresentation from './SignInFormPresentation.vue';
import type { SignInFormState } from './SignInForm.types';
...
describe('SignInFormPresentation', () => {
  it('should render sign in form with correct elements', () => {
    const { getByTestId } = render(SignInFormPresentation, {
      props: {
    ...
    getByTestId('auth-sign-in-button');
    getByTestId('auth-sign-up-link');
  });
  ...
  it('should email input fire change event when value is changed', async () => {
    const { emitted } = render(SignInFormPresentation, {
      props: {
        formState: { ...mockFormState },
      },
    });
  ...
  it('should emit submit event when form is submitted', async () => {
    const { getByTestId, emitted } = render(SignInFormPresentation, {
      props: {
        formState: { ...mockFormState },
      },
    });
